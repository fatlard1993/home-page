...
const homePage = {
	...
	init: function () {
		...
		homePage.searchBar.onblur = () => {
			homePage.searchBar.placeholder = 'Search (Press / to focus)';
		};
		...
		socketClient.on('search', search => {
			...
			const relativeOrder = relevancy
				.sort(homePage.bookmarks.__sortOrder, search.keyword)
				.slice(0, 1)
				.concat(relevancy.sort(suggestions.concat(homePage.history)).slice(0, 5));

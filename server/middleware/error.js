...
module.exports = {
	four0four: function(req, res, next){
		...
		var htmlPath = process.env.DIST ? `${__dirname}/../resources/html` : `${__dirname}/../../client/public/html`;

		Exec(`(cd ${htmlPath} && sed -e 's/XXX/${(Headers[err.status] || err.status)}/g' -e 's/YYY/${detail}/g' error.html > ./error_${err.status}.html)`, function(){
